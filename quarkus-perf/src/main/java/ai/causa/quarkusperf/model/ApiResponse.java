package ai.causa.quarkusperf.model;
 ...
 * {@code chaos.http.large-response.kb}. This forces Quarkus RESTEasy to materialise
 * the full payload in heap memory on the I/O thread before writing it to the socket,
 * reproducing the memory pressure that arises from HTTP keep-alive misconfiguration
 * (large responses held alive on many concurrent connections simultaneously).
 */
public class ApiResponse<T> {
     ...
     * Kept as a plain String so Jackson writes it inline in the response body,
     * keeping the full allocation live on the I/O thread during serialisation.
     */
