...
import ai.causa.quarkusperf.service.BookingService;
...
public class BookingResource {
    ...
    public Response createBooking(BookingRequest request) {
        ...
        LOG.infof("[%s] POST /api/bookings passenger=%s route=%s→%s",
        ...
        try {
            ...
            LOG.errorf("[%s] Booking failed: %s", correlationId, e.getMessage());
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity(ApiResponse.error("Booking processing failed", "INTERNAL_ERROR", correlationId))
                    .build();
        }
    }
