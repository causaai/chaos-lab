...
/**
 * Repository for airline Bookings backed by JDBC (Agroal pool).
 *
...
public class BookingRepository {
    ...
    public void ensureSchema() {
        try (Connection c = dataSource.getConnection();
            ...
            LOG.info("Booking schema initialised successfully");
        } catch (SQLException e) {
            LOG.errorf("BookingRepository schema init failed: %s", e.getMessage());
    ...
    public Booking save(Booking booking) {
        ...
        try {
            ...
            LOG.warnf("save booking failed: %s", e.getMessage());
    ...
    public Optional<Booking> findByRef(String bookingRef) {
        ...
        try {
            ...
            LOG.warnf("findByRef failed: %s", e.getMessage());
            return Optional.empty();
        } finally {
            maybeClose(conn);
        }
    ...
    public List<Booking> findByPassenger(String passengerId) {
        ...
        try {
            ...
            LOG.warnf("findByPassenger failed: %s", e.getMessage());
        } finally {
            maybeClose(conn);
        }
        return result;
    ...
    private Connection acquireConnection() {
        try {
            ...
            if (chaosConfig.db().leakEnabled()) {
                LOG.warn("[CHAOS] Booking connection acquired and intentionally NOT returned to pool. " +
                        "Pool exhaustion will occur under sustained load.");
            }
            return conn;
        } catch (SQLException e) {
            LOG.errorf("[CHAOS] Failed to acquire DB connection (booking): %s", e.getMessage());
    ...
    private void maybeClose(Connection conn) {
        if (!chaosConfig.db().leakEnabled() && conn != null) {
            try {
                ...
                LOG.warnf("Failed to close booking connection: %s", e.getMessage());
            }
        }
    }
